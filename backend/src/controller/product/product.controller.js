...
const editProduct = [
    async (req, res) => {
        try {
            ...
            if (updatedProduct) {
                return res.json({ status: "success", message: "Product updated successfully", data: updatedProduct });
            } else {
                return res.json({ status: "failed", message: "Unable to update product", data: [] });
            }
        } catch (err) {
            console.error(err);
            return res.json({ status: "failed", message: "Unable to update product", data: [] });
